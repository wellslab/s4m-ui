    ...
    <b-tabs card pills v-model="selectedTabIndex" align="center">
        ...
        <b-row class="small justify-content-center">
            <b-col md="3">
                ...
                <b-form-select v-model="sampleGroupsTab.selectedView" size="sm" class="m-1">
                    <b-form-select-option value="PCA by sample group">
                        {{ atlasType === 'training' ? 'Latent space by sample group' : 'PCA by sample group' }}
            ...
            <b-col class="overflow-auto" @mousemove="updateMousePosition"> 
                <b-card v-show="sampleGroupsTab.selectedView!='Customise sample groups'" border-variant="light">
                    <h5 class="text-center">{{ atlasType === 'training' ? 'Latent space by sample group' : 'PCA by sample group' }}
        ...
        <b-row class="small justify-content-center">
            <b-col md="3">
                ...
                Plot type:
                <b-form-select size="sm" v-model="geneExpressionTab.selectedPlotType" :options="customPlotTypes"  
        ...
        <!-- Projection Tab -->
        <b-tab v-if="atlasType !== 'training'" title="Projection"> <!-- disable this tab for training atlas-->
        <b-row class="small">
            <b-col md="3">
                    ...
                    <b-card border-variant="light">
                        ...
                        <div style="font-size:13px"> <!-- same style as b-card-text which is basically <p> that shouldn't be outside ul -->
                            <div v-if="atlasType === 'training'">
                                ...
                                the normalised expression file contains normalised expression values generated by scVI model (note: large file).
                            </div>
                            <div v-else>
                                You can download the data used for the atlas here. The samples matrix contains data about each sample, 
                                the genes matrix shows which genes were included in the atlas and which were left out, and 
                            ...
                            <ul class="mt-2">
                                <li v-for="item in customFileTypes" :key="item.key">
    ...
    <div class="px-3 py-2">
        <div v-if="atlasType === 'training'">
            ...
            <p>You can explore the atlas in several ways: <b>"Sample groups"</b> tab contains ways
                ...
                plot gene expression in the atlas in different ways. 
            </p>
            <p>On the plots, zoom in or out using your mouse, double-click on individual samples in the 3D plot of latent space
...
export default {
    ...
    computed: {
        // replace text of gene expression tab dropdown selection box for training atlas
        customPlotTypes() {
            return this.geneExpressionTab.plotTypes.map(type => {
                return {
                    value: type.value,
                    text: this.atlasType === 'training' && type.value === 'pca' ? 'Latent space' : type.text
                };
            });
        },

        // replace text in tools tab of file names
        customFileTypes() {
            return this.toolsTab.fileTypes.map(fileType => {
                if (this.atlasType === 'training') {
                    if (fileType.key === 'expression-file') {
                        return { ...fileType, name: 'normalised expression' };
                    } else if (fileType.key === 'coordinates') {
                        return { ...fileType, name: 'coordinates in latent space' };
                    }
                }
                return fileType;
            });
    ...
    methods: {
        ...
        layout() {
            ...
            // Determine axis titles based on atlasType
            const isTraining = this.atlasType === 'training';

            const xTitle = isTraining ? 'Latent Dimension 1' : `PC1 (${pca_variances[this.atlasType][0]}%)`;
            const yTitle = isTraining ? 'Latent Dimension 2' : `PC2 (${pca_variances[this.atlasType][1]}%)`;
            const zTitle = isTraining ? 'Latent Dimension 3' : `PC3 (${pca_variances[this.atlasType][2]}%)`;
        ...
        traces(type="sample type") {
            ...
            if (type=="sample type") {
                ...
                // Set colorbar title based on atlasType
                trace.marker.colorbar = {
                    title: self.atlasType === 'training' ? "Normalised expression" : "Rank normalised"
                };
